...
class SellerController extends GetxController {
  ...
  Future<void> getSeller({bool loadMore = false}) async {
    if (!loadMore) {
      isLoading.value = true;
      currentPage = 1;
    }
    final url =
        Uri.parse("https://demo.alorferi.com/api/users?page=$currentPage");
  ...
  Future<void> sellerProduct(String sellerId) async {
    final url =
        Uri.parse("https://demo.alorferi.com/api/users/$sellerId/products");
    var response = await http.get(url);
