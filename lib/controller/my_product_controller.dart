...
class MyProductController extends GetxController {
  ...
  Future<void> createProductWithImage(
      Map<String, dynamic> product, File? imageFile) async {
    try {
      isLoading.value = true;
      var token = await TokenSharePrefences.loadToken();
      ...
      request.fields
          .addAll(product.map((key, value) => MapEntry(key, value.toString())));
  ...
  Future<void> updateProduct(
      String id, Map<String, dynamic> updatedProduct, File? imageFile) async {
    try {
      isLoading.value = true;
      var token = await TokenSharePrefences.loadToken();
      ...
      request.fields.addAll(
          updatedProduct.map((key, value) => MapEntry(key, value.toString())));
